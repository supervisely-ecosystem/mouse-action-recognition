...
def get_or_create_session(api: sly.Api) -> ModelApi:
    ...
    task_info = api.task.start(agent_id=agent.id, workspace_id=env.workspace_id(), module_id=module_id, task_name=RT_DETR_SESSION_NAME, app_version="docker-image-deploy-api-2", is_branch=True)
