...
class MaximalBBoxSlidingWindow2(VideoSlidingWindow):
    ...
    def __iter__(self):
        for detection in self.ann_iterator:
            ...
            max_frame_idx = self.vr.get_frames_num() - 1
            frame_indices = list(range(self.frame_index, min(self.frame_index + self.num_frames*self.frame_sample_rate, max_frame_idx), self.frame_sample_rate))
            buffer = self.vr.get_batch(frame_indices).asnumpy()
            w, h = buffer.shape[2], buffer.shape[1]
